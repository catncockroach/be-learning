...
### 4. Join
**JOIN** dùng để kết hợp các dòng từ nhiều bảng dựa trên một điều kiện liên quan

Các loại JOIN thường gặp trong PostgreSQL:
- INNER JOIN: chỉ lấy các dòng có dữ liệu khớp ở cả hai bảng.
- LEFT JOIN: lấy toàn bộ dòng của bảng bên trái, nếu không có dòng tương ứng bên phải thì các cột bên phải sẽ là NULL.
- RIGHT JOIN: ngược lại với LEFT JOIN, giữ toàn bộ dòng của bảng bên phải.
- FULL OUTER JOIN: giữ toàn bộ dòng của cả hai bảng; phần không khớp sẽ có giá trị NULL.
...
**Subquery** là một câu `SELECT` được lồng bên trong một câu query.   
Subquery thường được dùng trong `WHERE`, `FROM` hoặc `SELECT` để tạo ra dữ liệu trung gian cho query bên ngoài.

Ví dụ, tìm nhân viên có lương cao hơn **mức lương trung bình**:
...
```
Query bên trong được thực thi để tính mức trung bình trước (avg), sau đó query bên ngoài dùng kết quả đó để lọc nhân viên.

Kết quả:  ![alt text](images/image-9.png)
...
- Subquery trả về nhiều dòng: thường kết hợp với `IN`, `ANY`, `ALL`.
  ```sql
  SELECT name
  FROM employees
...
- Subquery trong `FROM`: tạo một bảng tạm thời để query tiếp. Dạng này thường được gọi là **derived table**.
  ```sql
...
> Subquery giúp biểu diễn các query phức tạp thành nhiều bước logic.  
Nhiều trường hợp có thể viết lại bằng `JOIN`,.. để query dễ đọc và dễ tối ưu hơn.  

### 6. Index

Index tăng tốc độ tìm kiếm bằng cách tạo cấu trúc dữ liệu tra cứu (B-tree) thay vì phải quét toàn bộ bảng (sequential scan).  
Đánh đổi: index tốn thêm dung lượng và làm chậm thao tác ghi (`INSERT`/`UPDATE`/`DELETE`)  
khi đó phải cập nhật cả index.
...
> Với bảng chỉ có vài dòng, PostgreSQL query planner ước lượng quét toàn bộ bảng (`Seq Scan`) rẻ hơn dùng index, nên vẫn chọn seq scan dù index đã tồn tại.  
> Index chỉ phát huy hiệu quả rõ rệt khi bảng có lượng dữ liệu đủ lớn.

### 7. Partition

Partition chia một bảng logic thành nhiều bảng vật lý nhỏ hơn (partition) dựa trên một quy tắc (range, list hoặc hash).  
Giúp truy vấn/maintain hiệu quả hơn trên bảng có dữ liệu rất lớn (ví dụ: chỉ quét đúng partition theo tháng thay vì toàn bộ dữ liệu nhiều năm).
...
> Kết quả thực hành: mỗi dòng tự được lưu vào đúng partition theo `order_date`.
![alt text](./images/image-11.png)

### 8. Transaction

Transaction gom nhiều câu lệnh thành một đơn vị "tất cả hoặc không gì cả" (atomic),  
Tuân theo ACID (Atomicity, Consistency, Isolation, Durability). 
- Hoặc tất cả xong, hoặc không thực hiện gì cả
- Đảm bảo từ trạng thái hợp lệ -> hợp lệ (Nhất quán)
- Các transaction chạy đồng thời, nhưng không thay đổi thay đổi chưa commit của các transaction khác.
- Sau khi commit, các thay đổi lưu vĩnh viễn (bền vững)  

`COMMIT` lưu thay đổi vĩnh viễn, `ROLLBACK` hủy toàn bộ thay đổi trong transaction.
